...
const EMPTY_FORM = { name: "", designation: "", image_url: "" };
...
export function AdvisorManager() {
    ...
    const openEdit = (a: Advisor) => {
        setEditing(a);
        setForm({ name: a.name, designation: a.designation, image_url: a.image_url || "" });
        setShowModal(true);
    };
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setUploading(true);
        try {
            const res = await api.uploadImageFile(file, `advisor-${form.name}`);
    ...
    const handleSave = async () => {
        ...
        try {
            if (editing) {
                const updated = await api.updateAdvisor(editing.id, form);
                setAdvisors(prev => prev.map(a => a.id === editing.id ? updated : a));
                showToast("✅ Advisor updated!");
            } else {
                const created = await api.addAdvisor(form);
    ...
    const handleDrop = async (dropIdx: number) => {
        ...
        const updated = newList.map((a, i) => ({ ...a, sort_order: i + 1 }));
        setAdvisors(updated);
        setDragIdx(null); setOverIdx(null);
        try {
            await api.reorderAdvisors(updated.map(a => ({ id: a.id, sort_order: a.sort_order })));
    ...
    const PLACEHOLDER = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Ccircle cx='50' cy='50' r='50' fill='%23CBD5E1'/%3E%3Ccircle cx='50' cy='38' r='16' fill='%2394A3B8'/%3E%3Cellipse cx='50' cy='85' rx='28' ry='20' fill='%2394A3B8'/%3E%3C/svg%3E";
    ...
    return (
        <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
                <div>
                    <h3 style={{ margin: 0, fontSize: "22px", fontWeight: "700", color: "#1e293b" }}>Advisor Panel</h3>
                    <p style={{ margin: "4px 0 0", color: "#64748b", fontSize: "14px" }}>{advisors.length} advisors · Drag to reorder</p>
                </div>
            ...
            <div style={card({ overflow: "hidden" })}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                        <tr style={{ background: "#f8fafc", borderBottom: "2px solid #e2e8f0" }}>
                            {["#", "", "Name", "Designation", "Actions"].map(h => (
                        ...
                        {advisors.map((a, idx) => (
                            <tr key={a.id} draggable
                                ...
                                <td style={{ padding: "12px 16px" }}>
                                    <img src={a.image_url || PLACEHOLDER} alt={a.name}
                                ...
                                <td style={{ padding: "12px 16px", color: "#64748b", fontSize: "14px" }}>{a.designation || "—"}</td>
                                <td style={{ padding: "12px 16px" }}>
                                    <div style={{ display: "flex", gap: "8px" }}>
                                        <button onClick={() => openEdit(a)} style={{ background: "#f1f5f9", border: "none", borderRadius: "8px", padding: "7px 14px", cursor: "pointer", fontSize: "13px", fontWeight: "600", color: "#475569" }}>✏️ Edit</button>
                                        <button onClick={() => setDeleteConfirm(a.id)} style={{ background: "#fef2f2", border: "none", borderRadius: "8px", padding: "7px 14px", cursor: "pointer", fontSize: "13px", fontWeight: "600", color: "#ef4444" }}>🗑️ Delete</button>
                                    </div>
                                </td>
                            </tr>
                        ))}
                ...
                {advisors.length === 0 && <div style={{ padding: "60px", textAlign: "center", color: "#94a3b8" }}>No advisors yet. Click "Add Advisor" to get started.</div>}
            </div>
            ...
            {showModal && (
                <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
                    <div style={card({ width: "100%", maxWidth: "480px", padding: "32px", maxHeight: "90vh", overflowY: "auto" })}>
                        <h3 style={{ margin: "0 0 24px", fontSize: "20px", fontWeight: "700", color: "#1e293b" }}>{editing ? "Edit Advisor" : "Add New Advisor"}</h3>

                        {(["name", "designation"] as const).map(field => (
                            <div key={field} style={{ marginBottom: "18px" }}>
                                <label style={{ display: "block", fontSize: "13px", fontWeight: "600", color: "#374151", marginBottom: "6px" }}>
                                    {field === "name" ? "Full Name *" : "Designation"}
                                </label>
                                <input type="text" value={form[field]} onChange={(e) => setForm({ ...form, [field]: e.target.value })}
                                    placeholder={field === "name" ? "e.g. Prof. Kamal Hossain" : "e.g. Principal"}
            ...
            {deleteConfirm && (
                <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
                    <div style={card({ width: "100%", maxWidth: "400px", padding: "32px", textAlign: "center" })}>
                        ...
                        <h3 style={{ margin: "0 0 8px", fontSize: "20px", fontWeight: "700", color: "#1e293b" }}>Delete Advisor?</h3>
